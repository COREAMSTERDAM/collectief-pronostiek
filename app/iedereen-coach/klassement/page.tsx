      ...
      <div className="ucl-container">
        ...
        <section className="ucl-card mb-8">
          ...
          >
            ...
            <span
              className={`shrink-0 text-2xl font-black text-emerald-300 transition-transform duration-200 ${
            ...
            <div className="ucl-card overflow-hidden p-0">
              {rows.map((row, index) => (
                <Link
                  ...
                  className={`group grid grid-cols-[minmax(0,1fr)_5.5rem_1rem] items-center gap-2 border-b border-white/[0.075] px-3 py-3 transition last:border-b-0 hover:bg-white/[0.07] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70 ${
                    row.is_current_user
                      ? "ucl-ranking-current"
                      : ""
                  }`}
                >
                  ...
                  <span
                    className="justify-self-end text-lg font-black text-slate-600 transition group-hover:translate-x-1 group-hover:text-emerald-300"
                    aria-hidden="true"
                  >
                    ›
                  </span>
              ...
              {history.map((match) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-black tabular-nums text-emerald-200">
...
function CoachAvatar({
  ...
  const ringClass =
    position === 1
      ? "border-emerald-300 ring-2 ring-emerald-300/35"
      : position === 2
  ...
  return (
    <div
      className={`flex shrink-0 items-center justify-center overflow-hidden rounded-full border bg-gradient-to-br from-emerald-500/25 to-emerald-500/20 font-black text-white shadow-lg shadow-slate-950/20 ${sizeClass} ${ringClass}`}
    >
      {avatarUrl ? (
        <img
          src={avatarUrl}
...
function CoachPodiumCard({
  ...
  const podiumClass =
    position === 1
      ? "border-emerald-300/25 bg-gradient-to-b from-emerald-400/30 to-emerald-700/20 text-emerald-100"
      : position === 2
        ? "border-slate-300/20 bg-gradient-to-b from-slate-300/20 to-slate-700/20 text-slate-100"
        : "border-emerald-400/20 bg-gradient-to-b from-emerald-400/20 to-emerald-800/20 text-emerald-100";

  return (
    <Link
      href={`/profiel/${player.user_id}`}
      aria-label={`Bekijk het profiel van ${player.coach_name}`}
      className={`w-1/3 rounded-2xl border p-3 text-center backdrop-blur-xl transition hover:-translate-y-1 hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70 ${cardClass} ${
